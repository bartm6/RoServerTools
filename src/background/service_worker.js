...
function enqueueJoinInfoJob(job) {
  while (joinInfoQueue.length >= JOININFO_MAX_QUEUE) {
    ...
    try {
      ...
      e.status = 503;
      dropped?.reject?.(e);
    } catch (error) { rsdWarnOnce("src/background/service_worker.js:32", error); }
  }
...
async function cleanupJoinInfoUserAgentRule() {
  ...
  } catch (error) { rsdWarnOnce("src/background/service_worker.js:69", error); } finally {
...
async function enableJoinInfoUserAgentRule() {
  ...
  } catch (error) { rsdWarnOnce("src/background/service_worker.js:106", error); }
}
...
async function disableJoinInfoUserAgentRule() {
  ...
  } catch (error) { rsdWarnOnce("src/background/service_worker.js:118", error); } finally {
    joinInfoUaRuleEnabled = false;
  }
}
...
function releaseJoinInfoUserAgent() {
  ...
  joinInfoUaDisableTimer = setTimeout(() => {
    ...
    if (joinInfoUaRefCount === 0 && joinInfoInFlight === 0 && joinInfoQueue.length === 0) {
      disableJoinInfoUserAgentRule().catch((error) => rsdWarnOnce("src/background/service_worker.js:disableJoinInfoUserAgentRule", error));
    }
  }, 800);
}
  ...
  while (joinInfoInFlight < JOININFO_MAX_INFLIGHT && joinInfoQueue.length > 0) {
    ...
    if (isServerTemporarilyBad(job.placeId, job.serverId)) {
      try {
        ...
        e.status = 504;
        job.reject(e);
      } catch (error) { rsdWarnOnce("src/background/service_worker.js:265", error); }
      continue;
    }
    ...
    (async () => {
      try {
        ...
        joinInfoInFlight--;
        pumpJoinInfoQueue().catch((error) => rsdWarnOnce("src/background/service_worker.js:pumpJoinInfoQueue", error));
      }
    })();
  }
}
...
function isAllowedCatalogSender(sender) {
  const url = sender?.url || sender?.tab?.url || "";
  return /^https:\/\/www\.roblox\.com\/(?:[a-z]{2}(?:-[a-z]{2})?\/)?(?:catalog|bundles)\/\d+/.test(
    url,
  );
}

async function fetchCatalogItemDetails(itemKind, itemId) {
  const kind = itemKind === "bundle" ? "Bundle" : "Asset";
  const id = parseInt(itemId, 10);
  if (!Number.isFinite(id) || id <= 0) {
    const e = new Error("Invalid catalog item id");
    e.status = 400;
    throw e;
  }
  ...
  if (!resp.ok) {
    const errText = await resp.text().catch(() => "");
    const e = new Error(`catalog item details failed: ${resp.status}`);
    e.status = resp.status;
    e.body = errText;
    throw e;
  }
...
try {
  chrome.runtime.onInstalled?.addListener(() => {
    cleanupJoinInfoUserAgentRule().catch((error) => rsdWarnOnce("src/background/service_worker.js:cleanupJoinInfoUserAgentRule", error));
  });
...
} catch (error) { rsdWarnOnce("src/background/service_worker.js:352", error); }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  (async () => {
    try {
      ...
      if (message.action === "fetchCatalogItemDetails") {
        if (!isAllowedCatalogSender(sender))
          return sendResponse?.({
            success: false,
            error: "Blocked: sender not allowed",
          });

        const itemKind = String(message.itemKind || "");
        const itemId = String(message.itemId || "");
        if (!itemId || !["asset", "bundle"].includes(itemKind))
          return sendResponse?.({
            success: false,
            error: "Missing or invalid itemKind/itemId",
          });

        try {
          const data = await fetchCatalogItemDetails(itemKind, itemId);
